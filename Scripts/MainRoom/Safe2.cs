...
public class Safe2 : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        {
            GUI.Label(new Rect(Screen.width / 2 - 75, Screen.height - 100, 150, 30), "The door is locked.");
        }
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            enter = true;
            if (issolved) //change to true to open safe
            
            {
                Debug.Log("key");
                lockedstate = false;
            }
            /*else if (GameObject.Find("InventoryController").GetComponent<Inventory>().enableClue1 == false)//if player dont have key but in trigger area
            {
                lockedstate = true; //door is locked
            }*/
        }
